        ...
        if self.accelerator.is_main_process and self.log_completions:
            ...
            if report_to_wandb:
                ...
                if self._logs.get('image'):
                    ...
                    # ]
                    # wandb_table['all_images'] = [
                    #     wandb.Image([load_pil_img(path) for path in img_list if path is not None])
                    #     if img_list else None
                    #     for img_list in self._logs['image']
                    # ]
                    # 修改后的代码，直接将图片路径列表传递给 wandb.Image
                    # wandb_table['all_images'] = [
                    #     wandb.Image(img_list) if img_list else None
                    #     for img_list in self._logs['image']
                    # ]
                    # 正确做法：为表格的单元格创建一个 wandb.Image 对象的列表
                    # wandb_table['all_images'] = [
                    #     [wandb.Image(path) for path in img_list if path is not None]
                    #     if img_list else None
                    #     for img_list in self._logs['image']
                    # ]
                    wandb_table['all_images'] = [
                        # 最终修正代码：从字典 path 中提取 'image' 键对应的值
                        [wandb.Image(path['image']) for path in img_list if path and path.get('image')]
                        if img_list else None
                        for img_list in self._logs['image']
                    ]
