...
class Template(ProcessorMixin):
    ...
    def _preprocess_inputs(
        ...
        # Apply Albumentations-based data augmentation during training
        if images and self.mode == 'train':
            try:
                ...
                for img in images:
                    if isinstance(img, Image.Image):
                        arr = np.array(img)
                        augmented = trans(image=arr)['image']
                        # before_path = self._save_pil_image(Image.fromarray(arr))
                        # after_path = self._save_pil_image(Image.fromarray(augmented))
                        # logger.info(f'[AUG] saved: {before_path} -> {after_path}')
                        # print(f'[AUG] saved: {before_path} -> {after_path}')
                        # print(before_path)
                        # print(after_path)
                ...
                print(f"Error during image augmentation: {e}")  # 打印详细错误信息
